<script lang="ts">
  ...
  let metaPrintStartTime = $state(0);
  let metaEstimatedTime = $state(0);
  let estimatedDate = $state(Date.now());
  let apiUrl = bootParams.moonrakerApi;

  let loadDialog = $state(false);
  let selectedFileEntry = $state(0);
  ...
  let currentSort: sortVariant = $state(sortVariant.NameDesc);
  ...
  let fileEntries: FileListEntry[] = $state([]);
...
{#if confirmCancelPrint}
  <div class="bg-opacity-50 absolute flex h-full w-full items-center justify-center bg-black">
    <div class="bg-opacity-50 flex flex-col items-center justify-center gap-4 rounded-lg border-neutral-600 bg-neutral-700 p-4 drop-shadow-md backdrop-blur">
