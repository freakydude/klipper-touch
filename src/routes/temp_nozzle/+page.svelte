<script lang="ts">
  ...
  let presetArr = [0, 110, 190, 210, 230, 250];
...
<div class="flex w-full flex-col flex-wrap justify-between gap-2 border-t-2 border-red-400 bg-neutral-800 p-1">
  ...
  <div class="flex flex-col flex-wrap items-start justify-start">
    <div class="flex flex-col flex-wrap">
      <div class="flex justify-end border-neutral-600 text-sm text-white">
        <p class="px-1 text-red-400">Current</p>
        <p class="flex-grow pl-2 pr-1 text-end">{$nozzleTemp.toFixed(0)} °C</p>
      </div>
      <div class="flex justify-end border-neutral-600 text-sm text-white">
        <p class="px-1 text-red-400">Target</p>
        <p class="flex-grow pl-2 pr-1 text-end">{$nozzleTempTarget.toFixed(0)} °C</p>
      </div>
    </div>
    <div class="flex flex-col flex-wrap items-stretch justify-start gap-1 pt-1">
      <button
        class="flex rounded border-l-4 border-red-400 bg-neutral-700 px-1 py-2 text-white hover:bg-neutral-500"
        on:click={() => changeNozzleTemperature(distance)}
      >
